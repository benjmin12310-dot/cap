...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'leave') {
        ...
        if ($id > 0 && $start && $end) {
            $stmt = $conn->prepare("UPDATE doctors SET is_active=0, leave_start=?, leave_end=? WHERE id=?");
            ...
            $dr->bind_param('i', $id); $dr->execute();
            $drname = $dr->get_result()->fetch_assoc()['full_name'] ?? 'Doctor';
            $dr->close();
    ...
    if ($action === 'reinstate') {
        $id = intval($_POST['doctor_id'] ?? 0);
        if ($id > 0) {
            $stmt = $conn->prepare("UPDATE doctors SET is_active=1, leave_start=NULL, leave_end=NULL WHERE id=?");
    ...
    if ($action === 'delete') {
        ...
        // Fetch doctor name for verification
        $dr = $conn->query("SELECT full_name FROM doctors WHERE id=$id")->fetch_assoc();
        if (!$dr) {
...
");
if ($sr) foreach ($sr->fetch_all(MYSQLI_ASSOC) as $row) {
